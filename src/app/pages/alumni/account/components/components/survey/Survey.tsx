...
function Survey() {
  ...
  const getSurveyForm = async (userId: number) => {
    try {
      const response = await axios.get(`https://amsbackend-ghub.onrender.com/users/${userId}/survey`, {
        headers: {
      ...
      if (data.survey) {
        setTimeout(() => setFilled(true), 2000); // Assuming you have defined and set the 'setFilled' state elsewhere.
      } else {
        return;
      }
    } catch (error) {
