...
const Work = () => {
  ...
  const fetchExperiencesByUserId = async (userId: number) => {
    try {
      const response = await axios.get(`https://amsbackend-ghub.onrender.com/users/${userId}/experiences`);
  ...
  const postExperiencesByUserId = async (data: any, userId: number) => {
    try {
      const response = await axios.post(`https://amsbackend-ghub.onrender.com/experiences/${userId}`, data, {
        headers: {
